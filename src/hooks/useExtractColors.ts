...
export const useExtractColors = (
  ...
  const options = useMemo<Options>(() => ({ ...defaultOptions, ...customOptions }), [customOptions]);
...
async function extractDominantColors(imageUrl: string, options: Options): Promise<ExtractedColors> {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = function () {
      ...
      const colorMap: ColorMap = countColors(pixels, colorSimilarityThreshold, sortBy);
...
function formatColors(colors: ExtractedColors, options: Options): FormattedColors {
  ...
  switch (format) {
    case "hex":
      return {
        ...
        colors: colors.colors.slice(0, maxColors).map((color) => formatToHex(color)),
      };
    case "rgb":
      return {
        ...
        colors: colors.colors.slice(0, maxColors).map((color) => formatToRgb(color)),
      };
    case "hsl":
      return {
        ...
        colors: colors.colors.slice(0, maxColors).map((color) => formatToHsl(color)),
      };
    case "hsv":
      return {
        ...
        colors: colors.colors.slice(0, maxColors).map((color) => formatToHsv(color)),
      };
    default:
      return {
        ...
        colors: colors.colors.slice(0, maxColors).map((color) => formatToRgba(color)),
      };
  }
}
...
const parseHsvString = (hsvString: string): { h: number; s: number; v: number } => {
